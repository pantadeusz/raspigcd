...
void Machine::gotoXYZ( const Position &pos0, double velocity, int selectedAxes, double maxVelocityNoAccel, double ddt ) {
	...
	if ( maxVelocityNoAccel < 0 ) maxVelocityNoAccel = velocity;
		...
		while ( ( dSteps[0] != 0 ) || ( dSteps[1] != 0 ) || ( dSteps[2] != 0 ) ) {
			...
			if ( j < maxSteps / 2 ) {
				...
				if ( j >= ( maxSteps - stepsAccelerating ) ) {
					stepsAccelerating--;
					if (stepsAccelerating < 1) stepsAccelerating = 1;
				}
				//std::cout << " " << delay_us;
			}
			delay_us = dtA_us - ::sqrt(stepsAccelerating)*ddt;
			if ( delay_us > dtA_us ) delay_us = dtA_us;
			if ( delay_us < dt_us ) {
				delay_us = dt_us;
			}
...
int Machine::waitForEndstopTrigger() {
	...
	while (true) {
		...
		for (unsigned i = 0; i < state.size(); i++) {
			if (state[i] < prevState[i]) return i;
		}
		prevState = state;
