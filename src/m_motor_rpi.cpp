...
stepper_pi::stepper_pi() {
	...
	if ( configured < 0 ) throw "could not setup raspberry pi gpio";
}
...
void spindle_pi::setSpeed( double v ) {
	if ( conf.servopwm == 1 ) {
		if ( v < -1 ) dutyCycle = 0;
		else if ( v < 0 ) dutyCycle = 1000;
		else if ( v <= 1.0 ) dutyCycle = ( int )( ( v * 1000.0 ) + 1000 );
		else dutyCycle = 0;
	} else {
		if ( v > 0.5 ) {
			GPIO_SET = 1 << conf.pin;
...
void from_json( const nlohmann::json& j, SpindlePiConfig& p ) {
	try {p.pin                         = j.at("pin").get<int>();       } catch (...){}                       ; // 18
	try {p.servopwm                    = j.at("servopwm").get<int>();  } catch (...){}                       ; // 1
}

void from_json( const nlohmann::json& j, StepperPiConfig& p ) {
	try {p.dir                      = j.at("dir").get<double>(); } catch (...){}                    ; // 27
	try {p.en                       = j.at("en").get<double>();  } catch (...){}                    ; // 10
	try {p.step                     = j.at("step").get<double>();} catch (...){}                    ; // 22
}
...
void from_json( const nlohmann::json& j, ButtonsPiConfig& p ) {
	try {p.x                         = j.at("x").get<int>();       } catch (...){}                       ; 
	try {p.y                         = j.at("y").get<int>();       } catch (...){}                       ; 
	try {p.z                         = j.at("z").get<int>();       } catch (...){}                       ; 
	try {p.t                         = j.at("t").get<int>();       } catch (...){}                       ; 
}
